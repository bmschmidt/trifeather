import { Table, Uint8, Dictionary, Float32, Utf8, Int16, Int8, vectorFromArray, tableFromArrays } from 'apache-arrow'
import { shuffle, sum, range, extent, min } from 'd3-array';



export function random_points(frame, fields, n_represented = 1, value = "feather", keep = [], names = ["category"], delim = "_") {
  /*
...
//          console.log({x, y})
  ...
  names.forEach((column_name, column_number) => {
    ...
    const classes = vectorFromArray(strings, new Dictionary(new Utf8(), dict_type))
  ...
  const my_table2 = new tableFromArrays({
    ...
    ...keep.reduce((acc, d, i) => ({...acc, [d]: vectorFromArray(keepers[i], new Utf8())}), {}),
