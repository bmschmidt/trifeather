...
export default class TriFeather {
  ...
  */
  static from_feature_collection(
    ...
    if (options.clip_to_sphere) {
      ...
      for (let feature of projected.features) {
        const new_coords = clip.intersection(
          feature.coordinates,
          clip_shape.coordinates
        );
        if (
          projected.type == "Polygon" &&
          typeof (new_coords[0][0][0] != "numeric")
        ) {
    ...
    for (let feature of projected.features) {
      ...
      for (let polygon of loc_coordinates) {
        ...
        const r = new Float32Array(coords.flat(3));
        //        console.log({r})
        const bigint_coords = new Float64Array(r.buffer);
        // Reduce to the indices of the master lookup table.
        for (let vertex of vertices) {
          all_vertices[all_vertices.length] = coord_indices.get(
            bigint_coords[vertex]
          );
          //          console.log(bigint_coords[vertex], all_vertices[all_vertices.length])
        }
  ...
  static infer_type(array, dictionary_threshold = 0.75) {
    ...
    for (let el of array) {
      if (Math.random() > 200 / array.length) {
        continue;
      } // Only check a subsample for speed. Try
    ...
    if (strings > 0) {
      // moderate overlap
      if (seen.length < strings.length * 0.75) {
  ...
  prepare_features_for_regl() {
    ...
    for (let ix = 1; ix < this.t.numRows; ix++) {
  ...
  *[Symbol.iterator]() {
  ...
  static lookup_map_and_coord_buffer(geojson) {
    const all_coordinates = new Float32Array(
      geojson.features
        .filter((d) => d.geometry)
