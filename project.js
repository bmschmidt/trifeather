...
program
  ...
  .option(
    "-n, --names [names...]",
    `Column names to split count fields into using delimiter. E.g., {'white_non-hispanic': 31}
  ...
  .option(
    ...
    "Projection. String representing d3-geo or d3-geo-projection factory function. Will be scaled up by a factor of 1e12 to support zoom.",
...
const proj =
  d3geo[program.projection] ||
  geoAlbersUsaTerritories[program.projection] ||
  geoproj[program.projection];
...
const delimiter =
  program.opts()["delimiter"] || names.length == 1
    ? "ARBITRYAR STRING_THAT_WONT_OCCURR IN TEXT"
    : "_";
...
for (let fname of fnames) {
  ...
  const destname = fname
    .replace(".geojson", counts ? ".feather" : ".trifeather")
  ...
  */
  ...
  let trifeather = TriFeather.from_feature_collection(
    feature_collection,
    projection
  );
